...
export function HabitsPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  const totalChecks = habits.reduce((sum, habit) => sum + habit.completedDates.filter((date) => days.some((day) => dateKey(day) === date)).length, 0);
  const consistency = habits.length ? Math.round((totalChecks / (habits.length * 7)) * 100) : 0;

  return (
    <div className="habits-page page-enter">
      <header className="page-header">
        <div><span className="page-eyebrow">Powtarzalność ponad perfekcję</span><h1>Codzienny rytm</h1><p>Małe działania, które z czasem robią dużą różnicę.</p></div>
        <button className="button button--primary" type="button" onClick={() => setAddOpen(true)}><Plus size={17} /> Nowy rytuał</button>
      </header>

      <section className="habit-hero">
        <div className="habit-hero__message"><span><Sparkles size={17} /></span><div><small>Ten tydzień</small><h2>{consistency >= 70 ? "Dobry rytm — tak trzymaj" : "Liczy się następny mały krok"}</h2><p>Nie musisz nadrabiać wczoraj. Zadbaj o jedną rzecz dzisiaj.</p></div></div>
        <div className="habit-score"><div style={{ "--progress": `${consistency * 3.6}deg` } as React.CSSProperties}><strong>{consistency}%</strong></div><span>regularności</span></div>
      </section>

      <section className="panel habit-week-panel">
        <header className="panel__header"><div><span className="section-kicker"><Flame size={14} /> Ostatnie 7 dni</span><h2>Twój tydzień</h2></div><span className="habit-today-count">Dzisiaj {todayDone}/{habits.length}</span></header>
        <div className="habit-week-scroll">
          <div className="habit-week-grid" style={{ "--days": days.length } as React.CSSProperties}>
            ...
            {days.map((day) => <div className={dateKey(day) === today ? "habit-day-head habit-day-head--today" : "habit-day-head"} key={dateKey(day)}><span>{formatDayName(day)}</span><strong>{format(day, "d")}</strong></div>)}
            {habits.map((habit) => {
              ...
              return [
                ...
                ...days.map((day) => {
                  ...
                  return <button className={done ? "habit-day-cell habit-day-cell--done" : "habit-day-cell"} type="button" key={`${habit.id}-${key}`} onClick={() => toggleHabit(habit.id, key)} aria-label={`${habit.name}, ${format(day, "EEEE d MMMM", { locale: pl })}: ${done ? "wykonane" : "niewykonane"}`}>{done && <Check size={16} strokeWidth={3} />}</button>;
                }),
              ];
            })}
          </div>
      ...
      <div className="habit-insights">
        <article><span className="insight-number">{totalChecks}</span><div><strong>małych zwycięstw</strong><p>w ostatnich siedmiu dniach</p></div></article>
        <article><span className="insight-icon"><Flame size={21} /></span><div><strong>Najlepsza seria: {Math.max(0, ...habits.map(calculateStreak))} dni</strong><p>Regularność lubi prostotę.</p></div></article>
