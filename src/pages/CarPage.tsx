...
export function CarPage({ onToast }: CarPageProps) {
  ...
  const selectedDeadlines = useMemo(
    () =>
      vehicleDeadlines
        ...
        .sort((a, b) =>
          selectedVehicle
            ? deadlineOrder(a, selectedVehicle) - deadlineOrder(b, selectedVehicle)
            : 0,
        ),
  ...
  const fuelExpenses = selectedExpenses.filter(
    (expense) => expense.type === "fuel" && expense.liters,
  );
  ...
  const saveExpense = (event: FormEvent) => {
    ...
    const liters =
      expenseDraft.type === "fuel" && expenseDraft.liters
  ...
  const saveDeadline = (event: FormEvent) => {
    ...
    if (
      !selectedVehicle ||
      !deadlineDraft.title.trim() ||
      (!deadlineDraft.dueDate && !deadlineDraft.dueMileage)
    ) {
    ...
    <div className="life-module-page page-enter">
      <header className="page-header life-module-header">
        ...
        <button
          ...
          type="button"
          onClick={() => openExpenseCreate("fuel")}
        >
      ...
      <section className="garage-strip" aria-label="Twój garaż">
        {vehicles.map((vehicle) => (
          <button
            className={
              selectedVehicle?.id === vehicle.id ? "garage-card garage-card--active" : "garage-card"
            }
            type="button"
            key={vehicle.id}
          ...
          >
            <span className="garage-card__icon">
              <Car size={22} />
            </span>
            <div>
              <strong>{vehicle.name}</strong>
              <span>
                {vehicle.year} · {fuelLabels[vehicle.fuelType]}
              </span>
              <small>{vehicle.plate || "Bez numeru"}</small>
            </div>
            <span className="garage-card__mileage">
              {new Intl.NumberFormat("pl-PL").format(vehicle.mileage)} km
            </span>
          </button>
        ))}
        <button className="garage-add" type="button" onClick={openVehicleCreate}>
          <Plus size={20} />
      ...
      {selectedVehicle ? (
        <>
          <section className="module-stat-grid" aria-label="Podsumowanie pojazdu">
            <article className="module-stat-card module-stat-card--accent">
              ...
              <div>
                <span>Przebieg</span>
                <strong>{new Intl.NumberFormat("pl-PL").format(selectedVehicle.mileage)} km</strong>
                <small>
            ...
            <article className="module-stat-card">
              ...
              <div>
                <span>Średnia cena paliwa</span>
                <strong>
                  {averageFuelPrice
                    ? `${formatMoney(Math.round(averageFuelPrice), "PLN", hideAmounts)} / l`
                    : "Brak danych"}
                </strong>
                <small>
                  {totalFuelLiters
                    ? `${totalFuelLiters.toFixed(1).replace(".", ",")} l w historii`
                    : "Dodaj tankowanie z litrami"}
                </small>
              </div>
            </article>
            <article
              className={
                urgentDeadlines.length
                  ? "module-stat-card module-stat-card--warning"
                  : "module-stat-card"
              }
            >
              ...
              <div>
                ...
                <strong>
                  {nextDeadline?.dueDate
                    ? relativeDay(nextDeadline.dueDate)
                    : nextDeadline?.dueMileage
                      ? `${new Intl.NumberFormat("pl-PL").format(nextDeadline.dueMileage)} km`
                      : "Brak"}
                </strong>
                <small>{nextDeadline?.title ?? "Wszystko dopilnowane"}</small>
              </div>
            </article>
          </section>
          ...
          <div className="car-dashboard-grid">
              ...
              <section className="panel module-panel">
                <header className="module-panel__header">
                  ...
                  <div className="module-toolbar-actions">
                    <div className="module-segmented">
                      ...
                      <button
                        className={expenseFilter === "fuel" ? "active" : ""}
                        type="button"
                        onClick={() => setExpenseFilter("fuel")}
                      >
                        Paliwo
                      </button>
                      <button
                        className={expenseFilter === "service" ? "active" : ""}
                        type="button"
                        onClick={() => setExpenseFilter("service")}
                      >
                        Serwis
                      </button>
                    </div>
                ...
                {visibleExpenses.length ? (
                      ...
                      <article className="car-expense-row" key={expense.id}>
                        ...
                        <div className="car-expense-row__details">
                          {expense.mileage && (
                            <span>
                              <Gauge size={13} />{" "}
                              {new Intl.NumberFormat("pl-PL").format(expense.mileage)} km
                            </span>
                          )}
                          {expense.liters && (
                            <span>
                              <Fuel size={13} /> {expense.liters.toFixed(1).replace(".", ",")} l
                            </span>
                          )}
                        </div>
                  ...
                  <div className="module-empty">
                    <ReceiptText size={24} />
                    <strong>Brak kosztów w tym widoku</strong>
                    <span>Dodaj tankowanie, serwis albo inny wydatek.</span>
                  </div>
                )}
              </section>
            </div>
            ...
            <aside className="car-dashboard-side">
              <section className="panel module-panel vehicle-overview-card">
                <header>
                  <span
                    className="vehicle-overview-icon"
                    style={{ background: selectedVehicle.color }}
                  >
                    <Car size={24} />
                  </span>
                  <div>
                    <span>{selectedVehicle.plate || "Twój pojazd"}</span>
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => openVehicleEdit(selectedVehicle)}
                ...
                <div className="vehicle-date-grid">
                  <div>
                    ...
                    <strong>{formatShortDate(selectedVehicle.insuranceDate)}</strong>
                  </div>
                  <div>
                    <Wrench size={16} />
                    <span>Przegląd</span>
                    <strong>{formatShortDate(selectedVehicle.inspectionDate)}</strong>
                  </div>
                </div>
                <form className="mileage-update" onSubmit={saveMileage}>
                  <label className="field">
                    ...
                    <div>
                      ...
                      />
                      <span>km</span>
                      <button type="submit" aria-label="Zapisz przebieg">
                        <Check size={16} />
                      </button>
                    </div>
                  </label>
              ...
              <section className="panel module-panel deadlines-panel">
                <header className="module-panel__header">
                  <div>
                    <span className="section-kicker">
                      <CalendarClock size={14} /> Nie przegap
                    </span>
                    <h2>Terminy</h2>
                  </div>
                  <button
                ...
                <div className="deadline-list">
                  {selectedDeadlines.map((deadline) => {
                    ...
                    return (
                      <article
                        className={`${deadline.completed ? "deadline-row deadline-row--done" : "deadline-row"} ${due ? "deadline-row--due" : ""}`}
                        key={deadline.id}
                      >
                        <button
                          type="button"
                          onClick={() => toggleVehicleDeadline(deadline.id)}
                          aria-label={
                            deadline.completed
                              ? `Przywróć ${deadline.title}`
                              : `Ukończ ${deadline.title}`
                          }
                        >
                          <Check size={13} />
                        </button>
                        <div>
                          ...
                          <span>
                            {deadline.dueDate
                              ? `${relativeDay(deadline.dueDate)} · ${formatShortDate(deadline.dueDate)}`
                              : deadline.dueMileage
                                ? `przy ${new Intl.NumberFormat("pl-PL").format(deadline.dueMileage)} km`
                                : "Bez terminu"}
                          </span>
                        </div>
        ...
        <section className="panel module-panel module-empty module-empty--large">
          <Car size={29} />
          <strong>Twój garaż jest pusty</strong>
          <span>Dodaj pierwszy pojazd, aby prowadzić przebieg, koszty i terminy.</span>
          <button className="button button--primary" type="button" onClick={openVehicleCreate}>
      ...
      >
        <form className="form-grid" onSubmit={saveVehicle}>
          <label className="field field--prominent">
            <span>Nazwa w dashboardzie</span>
            <input
              autoFocus
              required
              value={vehicleDraft.name}
              onChange={(event) => setVehicleDraft({ ...vehicleDraft, name: event.target.value })}
              placeholder="np. Rodzinna Toyota"
            />
          </label>
          <div className="form-grid form-grid--3">
            <label className="field">
              ...
              <input
                ...
                onChange={(event) => setVehicleDraft({ ...vehicleDraft, make: event.target.value })}
              />
            </label>
            <label className="field">
              <span>Model</span>
              <input
                ...
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, model: event.target.value })
                }
              />
            </label>
            <label className="field">
              ...
              <input
                ...
                onChange={(event) => setVehicleDraft({ ...vehicleDraft, year: event.target.value })}
              />
            </label>
          </div>
          <div className="form-grid form-grid--3">
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, plate: event.target.value })
                }
              />
            </label>
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, mileage: event.target.value })
                }
              />
            </label>
            <label className="field">
              ...
              <select
                ...
                onChange={(event) =>
                  setVehicleDraft({
                    ...vehicleDraft,
                    fuelType: event.target.value as Vehicle["fuelType"],
                  })
                }
              >
          ...
          <div className="form-grid form-grid--2">
            <label className="field">
              <span>Ważność badania</span>
              <input
                type="date"
                required
                value={vehicleDraft.inspectionDate}
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, inspectionDate: event.target.value })
                }
              />
            </label>
            <label className="field">
              <span>Ważność ubezpieczenia</span>
              <input
                type="date"
                required
                value={vehicleDraft.insuranceDate}
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, insuranceDate: event.target.value })
                }
              />
            </label>
          </div>
          <div className="form-grid form-grid--2">
            <label className="field">
              ...
              <select
                value={vehicleDraft.visibility}
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, visibility: event.target.value as Visibility })
                }
              >
            ...
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setVehicleDraft({ ...vehicleDraft, color: event.target.value })
                }
              />
            </label>
          </div>
      ...
      <Modal
        ...
        title={expenseDraft.type === "fuel" ? "Dodaj tankowanie" : "Dodaj koszt"}
      ...
      >
        <form className="form-grid" onSubmit={saveExpense}>
          <div className="form-grid form-grid--2">
            <label className="field">
              ...
              <select
                ...
                onChange={(event) => {
                  const type = event.target.value as CarExpense["type"];
              ...
              >
                <option value="fuel">Tankowanie</option>
          ...
          <div
            className={
              expenseDraft.type === "fuel" ? "form-grid form-grid--3" : "form-grid form-grid--2"
            }
          >
            <label className="field">
              <span>Kwota (PLN)</span>
            ...
            <label className="field">
              ...
              <input
                type="number"
                min="0"
                value={expenseDraft.mileage}
                onChange={(event) =>
                  setExpenseDraft({ ...expenseDraft, mileage: event.target.value })
                }
              />
            </label>
            {expenseDraft.type === "fuel" && (
              <label className="field">
                <span>Liczba litrów</span>
                <input
                  inputMode="decimal"
                  value={expenseDraft.liters}
                  onChange={(event) =>
                    setExpenseDraft({ ...expenseDraft, liters: event.target.value })
                  }
                  placeholder="42,1"
                />
              </label>
        ...
        <form className="form-grid" onSubmit={saveDeadline}>
          <label className="field field--prominent">
            ...
            <input
              ...
              onChange={(event) =>
                setDeadlineDraft({ ...deadlineDraft, title: event.target.value })
              }
              placeholder="np. Wymiana opon"
            />
          </label>
          <div className="module-choice-divider">
            <span>Ustal datę, przebieg lub oba</span>
          </div>
          <div className="form-grid form-grid--2">
            <label className="field">
              <span>Termin</span>
              <input
                ...
                onChange={(event) =>
                  setDeadlineDraft({ ...deadlineDraft, dueDate: event.target.value })
                }
              />
            </label>
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDeadlineDraft({ ...deadlineDraft, dueMileage: event.target.value })
                }
                placeholder="75000"
              />
            </label>
