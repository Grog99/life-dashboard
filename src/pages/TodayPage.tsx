...
export function TodayPage({ onQuickAdd, onNavigate, onToast }: TodayPageProps) {
  ...
  const financeBudgets = useAdvancedStore((state) => state.financeBudgets);
  ...
  const tomorrowTasksCount = tasks.filter((task) => task.date === tomorrow && task.status === "todo").length;
  ...
  const monthlySpent = Math.abs(financeTransactions.filter((item) => item.currency === budgetCurrency && item.bookedOn.startsWith(monthPrefix) && item.amountMinor < 0).reduce((sum, item) => sum + item.amountMinor, 0));
  const totalBudget = financeBudgets.filter((item) => item.currency === budgetCurrency).reduce((sum, item) => sum + item.limitMinor, 0);
  const nextTrip = trips.filter((trip) => trip.status !== "archived" && trip.endDate >= today).sort((a, b) => a.startDate.localeCompare(b.startDate))[0];
    ...
    <div className="today-page page-enter">
      <section className="today-hero">
        ...
        <div className="today-hero__main">
          ...
          <h1>{greeting(now)}{displayName} <span aria-hidden="true">👋</span></h1>
          <p>
        ...
        <div className="today-hero__aside">
          <div className="progress-ring" role="progressbar" aria-label="Dzisiejszy postęp" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress} style={{ "--progress": `${progress * 3.6}deg` } as React.CSSProperties}>
            <div><strong>{progress}%</strong><span>dnia</span></div>
          </div>
          <div>
            <span>Dzisiejszy postęp</span>
            <strong>{todayDoneTasks.length} z {totalToday || 0} zadań</strong>
          </div>
        </div>
        ...
        <div className="energy-check">
          ...
          <div>
            <button className={energy === "low" ? "active" : ""} type="button" onClick={() => setEnergy("low")} aria-label="Mało energii" aria-pressed={energy === "low"}>🌙</button>
            <button className={energy === "medium" ? "active" : ""} type="button" onClick={() => setEnergy("medium")} aria-label="Energia w sam raz" aria-pressed={energy === "medium"}>🌤️</button>
            <button className={energy === "high" ? "active" : ""} type="button" onClick={() => setEnergy("high")} aria-label="Dużo energii" aria-pressed={energy === "high"}>⚡</button>
          </div>
        </div>
      </section>
      ...
      <div className="dashboard-grid">
        <div className="dashboard-column dashboard-column--main">
          ...
          <section className="panel agenda-panel">
            <header className="panel__header">
              <div>
                <span className="section-kicker"><CalendarDays size={14} /> Rytm dnia</span>
                <h2>Plan na dziś</h2>
              </div>
              <button className="button button--soft button--small" type="button" onClick={() => onQuickAdd("event")}><Plus size={15} /> Wydarzenie</button>
            </header>
            {agenda.length ? (
              <div className="timeline">
                {agenda.map((item) => {
                  ...
                  return (
                    <div className={`timeline-item timeline-item--${item.kind} ${past ? "timeline-item--past" : ""} ${current ? "timeline-item--current" : ""}`} key={`${item.kind}-${item.id}`}>
                      <time>{item.time}</time>
                      ...
                      <div className="timeline-card">
                        ...
                        <span className="event-kind">
                          {item.kind === "meeting" ? "Spotkanie" : item.kind === "focus" ? "Skupienie" : item.kind === "task" ? "Zadanie" : "Prywatne"}
                        </span>
                      </div>
                    </div>
                  );
              ...
              <div className="soft-empty soft-empty--center">
                ...
                <div><strong>Masz przestrzeń w kalendarzu</strong><span>Zaplanuj blok skupienia albo zostaw ten czas wolny.</span></div>
                <button className="button button--soft" type="button" onClick={() => onQuickAdd("event")}>Zaplanuj czas</button>
              </div>
            )}
          </section>
          ...
          <section className="panel habits-strip-panel">
            <header className="panel__header panel__header--compact">
              <div><span className="section-kicker"><Leaf size={14} /> Małe rzeczy</span><h2>Codzienny rytm</h2></div>
              <button className="text-button" type="button" onClick={() => onNavigate("habits")}>Zobacz tydzień <ChevronRight size={16} /></button>
            </header>
            <div className="habit-strip">
              {habits.slice(0, 3).map((habit) => {
                ...
                return (
                  <button className={done ? "habit-chip habit-chip--done" : "habit-chip"} type="button" key={habit.id} onClick={() => toggleHabit(habit.id, today)} aria-pressed={done}>
                    <span><Icon size={18} /></span>
                    <div><strong>{habit.name}</strong><small>{done ? "Gotowe na dziś" : habit.targetLabel}</small></div>
                    <span className="habit-chip__check">{done ? <Check size={14} /> : <Circle size={14} />}</span>
                  </button>
                );
              })}
            </div>
          ...
          <section className="panel life-overview-panel">
            <header className="panel__header panel__header--compact">
              <div><span className="section-kicker"><Sparkles size={14} /> Całe życie</span><h2>W skrócie</h2></div>
              <button className="icon-button" type="button" onClick={toggleHideAmounts} aria-label={hideAmounts ? "Pokaż kwoty" : "Ukryj kwoty"}>{hideAmounts ? <Eye size={17} /> : <EyeOff size={17} />}</button>
            </header>
            <div className="life-overview-grid">
              <button type="button" onClick={() => onNavigate("finance")}><span className="life-module-icon life-module-icon--finance"><WalletCards size={18} /></span><div><small>Budżet miesiąca · {budgetCurrency}</small><strong>{formatMoney(monthlySpent, budgetCurrency, hideAmounts)} / {formatMoney(totalBudget, budgetCurrency, hideAmounts)}</strong><span>{totalBudget ? Math.min(100, Math.round(monthlySpent / totalBudget * 100)) : 0}% wykorzystane</span></div><ChevronRight size={15} /></button>
              <button type="button" onClick={() => onNavigate("trips")}><span className="life-module-icon life-module-icon--trip"><Plane size={18} /></span><div><small>Następna podróż</small><strong>{nextTrip?.name ?? "Brak planów"}</strong><span>{nextTrip ? `${relativeDay(nextTrip.startDate)} · gotowe ${nextTrip.progress}%` : "Zaplanuj wyjazd"}</span></div><ChevronRight size={15} /></button>
        ...
        <aside className="dashboard-column dashboard-column--side">
          <section className="next-card">
            <div className="next-card__top">
              <span><AlarmClock size={15} /> {nextItem ? "Następne" : "Spokojny moment"}</span>
            ...
            {nextItem ? (
              <>
                ...
                <p>{nextItem.endTime ? `${nextItem.time}–${nextItem.endTime}` : nextItem.meta || "Zaplanowane na dziś"} · {relativeTime(today, nextItem.time)}</p>
                <button className="button button--light" type="button" onClick={() => {
                  setFocusSeconds(25 * 60);
                  setFocusRunning(true);
                  onToast("Timer skupienia wystartował");
                }}><Play size={16} fill="currentColor" /> Zacznij 25 min skupienia</button>
              </>
            ) : (
              <>
          ...
          {(focusRunning || focusSeconds !== 25 * 60) && (
            <section className="focus-timer-card">
              <div className="focus-timer-card__icon"><TimerReset size={20} /></div>
              <div><span>Tryb skupienia</span><strong>{focusLabel}</strong></div>
              <button className="icon-button" type="button" onClick={() => setFocusRunning((value) => !value)} aria-label={focusRunning ? "Pauza" : "Wznów"}>
                {focusRunning ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}
              </button>
              <button className="icon-button" type="button" onClick={() => { setFocusRunning(false); setFocusSeconds(25 * 60); }} aria-label="Wyzeruj timer"><RotateCcw size={17} /></button>
            </section>
          )}

          <section className="panel scratchpad-card">
            <header className="panel__header panel__header--compact">
              <div><span className="section-kicker"><Lightbulb size={14} /> Pod ręką</span><h2>Szybka notatka</h2></div>
              <span className="autosave"><span /> zapisuje się</span>
            </header>
            <textarea
              value={scratchpad}
            ...
            <footer>
              ...
              <button type="button" onClick={scratchToTask}><ListTodo size={14} /> Pierwszy wiersz → zadanie</button>
            </footer>
          </section>

          <section className="panel reminders-card">
            <header className="panel__header panel__header--compact">
              <div><span className="section-kicker"><Bell size={14} /> Nie zapomnij</span><h2>Przypomnienia</h2></div>
              <button className="icon-button" type="button" onClick={() => onQuickAdd("reminder")} aria-label="Dodaj przypomnienie"><Plus size={18} /></button>
            </header>
            <div className="reminder-list">
              ...
              {!activeReminders.length && (
                <div className="mini-empty"><Check size={17} /><span>Wszystko zapamiętane.</span></div>
              )}
            </div>
            <button className="panel-link" type="button" onClick={() => onQuickAdd("reminder")}><Plus size={14} /> Dodaj przypomnienie</button>
          </section>

          <section className="tomorrow-preview">
            <div><span>{format(addDays(now, 1), "EEEE", { locale: pl })}</span><strong>{format(addDays(now, 1), "d MMMM", { locale: pl })}</strong></div>
            <p>
              {tomorrowEventsCount} {polishPlural(tomorrowEventsCount, "wydarzenie", "wydarzenia", "wydarzeń")} · {tomorrowTasksCount} {polishPlural(tomorrowTasksCount, "zadanie", "zadania", "zadań")}
            </p>
            <button type="button" onClick={() => onNavigate("calendar")} aria-label="Otwórz kalendarz"><ArrowRight size={17} /></button>
          </section>
        </aside>
      </div>
    </div>
