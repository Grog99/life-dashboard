        ...
        <aside className="panel trips-list" aria-label="Twoje podróże">
          <header>
            <div>
              ...
              <strong>
                {trips.filter((trip) => trip.status !== "archived").length} aktywne plany
              </strong>
            </div>
            <button
              className="trips-list__add"
              type="button"
              onClick={() => setTripModalOpen(true)}
            ...
            <div className="trips-hero__main">
              <div className="trips-hero__badges">
                <select
                  ...
                  onChange={(event) =>
                    updateTrip(selectedTrip.id, { status: event.target.value as Trip["status"] })
                  }
              ...
              <p>
                <CalendarDays size={15} /> {tripDateRange(selectedTrip)} <span>·</span> {duration}{" "}
            ...
            {tripViews.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.id === "packing" && packing.length > 0 && (
                    <span>
              ...
              <section className="trips-metrics" aria-label="Podsumowanie podróży">
                <article>
                  ...
                  <span className="trips-mini-progress">
                    <i style={{ width: `${selectedTrip.progress}%` }} />
                  </span>
                </article>
                <article>
                ...
                <article>
                  ...
                  <div>
                    ...
                    <strong>{formatMoney(plannedTotal, selectedTrip.currency, hideAmounts)}</strong>
                  </div>
                  <small>
                    {hasBudget
                      ? `z ${formatMoney(selectedTrip.budgetMinor ?? 0, selectedTrip.currency, hideAmounts)}`
                ...
                <article>
                  ...
                  <small>
                    {packedCount} z {packing.length} gotowe
                  </small>
                </article>
              </section>
                  ...
                  return (
                    <article className="trips-day" key={date}>
                      <header>
                        ...
                        <button
                          type="button"
                          onClick={() => openItineraryModal(date)}
                  ...
                  {bookings.map((booking) => {
                    ...
                    return (
                      <article className="trips-booking" key={booking.id}>
                        <header>
                          <span
                            className={`trips-booking__icon trips-booking__icon--${booking.type}`}
                          >
                            <Icon size={20} />
                          </span>
                          <button
                            ...
                            onClick={() => updateTripBooking(booking.id, { paid: !booking.paid })}
                          >
                        ...
                        <footer>
                          ...
                          <strong>
                            {formatMoney(booking.amountMinor, selectedTrip.currency, hideAmounts)}
                          </strong>
                          <button
                            className="icon-button module-danger-icon"
                            type="button"
            ...
            <div className="trips-budget-grid">
              <section className="panel trips-budget-card">
                ...
                <div className="trips-budget-card__stats">
                  <div>
                    <span>Opłacono</span>
                    <strong>{formatMoney(paidTotal, selectedTrip.currency, hideAmounts)}</strong>
                  </div>
                  {hasBudget && (
                    <div className={isOverBudget ? "is-over" : ""}>
                      <span>{isOverBudget ? "Ponad budżet" : "Zostało"}</span>
                      <strong>
                        {formatMoney(Math.abs(remainingBudget), selectedTrip.currency, hideAmounts)}
                      </strong>
                    </div>
                  )}
                </div>
              ...
              <form className="trips-packing-add" onSubmit={addPacking}>
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setPackingCategory(event.target.value as PackingItem["category"])
                    }
                  >
                      ...
                      <div>
                        {items.map((item) => (
                          <button
                            className={
                              item.packed
                                ? "trips-pack-item trips-pack-item--done"
                                : "trips-pack-item"
                            }
                            type="button"
                            key={item.id}
