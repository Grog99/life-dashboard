...
const destinations: Array<{ view: ViewId; label: string; icon: typeof LayoutDashboard; keywords: string }> = [
  { view: "today", label: "Przejdź do: Dzisiaj", icon: LayoutDashboard, keywords: "dzisiaj dashboard plan" },
  { view: "tasks", label: "Przejdź do: Zadania", icon: CheckSquare2, keywords: "zadania lista skrzynka" },
  { view: "calendar", label: "Przejdź do: Kalendarz", icon: CalendarDays, keywords: "kalendarz wydarzenia tydzień" },
  ...
  { view: "finance", label: "Przejdź do: Finanse", icon: WalletCards, keywords: "finanse budżet transakcje konta" },
  { view: "trips", label: "Przejdź do: Podróże", icon: Plane, keywords: "podróże wakacje wyjazdy plan" },
...
export function CommandPalette({ open, onClose, onNavigate, onQuickAdd }: CommandPaletteProps) {
  ...
  useEffect(() => {
    ...
    const handleKey = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(
        paletteRef.current.querySelectorAll<HTMLElement>("button:not([disabled]), input:not([disabled])"),
  ...
  const results = useMemo(() => {
    ...
    return [
      ...tasks
        .filter((task) => `${task.title} ${task.category}`.toLocaleLowerCase("pl").includes(normalized))
        .slice(0, 3)
        .map((task) => ({ id: task.id, label: task.title, meta: `Zadanie · ${task.category}`, view: "tasks" as ViewId, icon: CheckSquare2 })),
      ...events
        .filter((event) => event.title.toLocaleLowerCase("pl").includes(normalized))
        .slice(0, 3)
        .map((event) => ({ id: event.id, label: event.title, meta: `Wydarzenie · ${event.date}`, view: "calendar" as ViewId, icon: CalendarDays })),
      ...notes
        .filter((note) => `${note.title} ${note.content}`.toLocaleLowerCase("pl").includes(normalized))
        .slice(0, 3)
        .map((note) => ({ id: note.id, label: note.title, meta: "Notatka", view: "notes" as ViewId, icon: NotebookPen })),
      ...financeTransactions.filter((item) => `${item.title} ${item.merchant} ${item.category}`.toLocaleLowerCase("pl").includes(normalized)).slice(0, 2).map((item) => ({ id: item.id, label: item.title, meta: `Finanse · ${item.category}`, view: "finance" as ViewId, icon: WalletCards })),
      ...trips.filter((item) => `${item.name} ${item.destination}`.toLocaleLowerCase("pl").includes(normalized)).slice(0, 2).map((item) => ({ id: item.id, label: item.name, meta: `Podróż · ${item.destination}`, view: "trips" as ViewId, icon: Plane })),
  ...
  return (
    <div className="command-backdrop" onMouseDown={onClose}>
      <div ref={paletteRef} className="command-palette" role="dialog" aria-modal="true" aria-label="Wyszukiwarka i komendy" onMouseDown={(event) => event.stopPropagation()}>
        <div className="command-search"><Search size={20} /><input ref={inputRef} aria-label="Szukaj w Pulsie" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && results[0]) { event.preventDefault(); go(results[0].view); } }} placeholder="Szukaj zadań, notatek albo przejdź do widoku…" /><kbd>ESC</kbd><button className="icon-button command-close" type="button" onClick={onClose} aria-label="Zamknij wyszukiwarkę"><X size={17} /></button></div>
        <div className="command-results">
          {query ? (
            <>
              <span className="command-section-label">Wyniki</span>
              {results.map((result) => { const Icon = result.icon; return <button key={`${result.view}-${result.id}`} type="button" onClick={() => go(result.view)}><span className="command-result-icon"><Icon size={17} /></span><div><strong>{result.label}</strong><small>{result.meta}</small></div><span className="command-enter">↵</span></button>; })}
              {!results.length && <div className="command-empty"><Search size={20} /><span>Nic nie znaleziono dla „{query}”</span></div>}
            </>
          ) : (
            <>
              <span className="command-section-label">Szybkie akcje</span>
              <button type="button" onClick={() => { onQuickAdd(); onClose(); }}><span className="command-result-icon command-result-icon--brand"><Plus size={17} /></span><div><strong>Dodaj nową rzecz</strong><small>Zadanie, wydarzenie, przypomnienie lub notatka</small></div><kbd>N</kbd></button>
              <span className="command-section-label">Przejdź do</span>
              {destinations.map((destination) => { const Icon = destination.icon; return <button key={destination.view} type="button" onClick={() => go(destination.view)}><span className="command-result-icon"><Icon size={17} /></span><div><strong>{destination.label}</strong></div></button>; })}
            </>
          )}
        </div>
        <footer className="command-footer"><span><Command size={13} /> Puls</span><span>Enter wybiera · Esc zamyka</span></footer>
      </div>
    </div>
  );
}
