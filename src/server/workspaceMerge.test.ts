...
describe("workspace three-way merge", () => {
  ...
  it("resolves a same-field edit conflict using the more recent updatedAt, not the local side", () => {
    const base = {
      notes: [{ id: "n1", title: "Original", updatedAt: "2026-01-01T00:00:00.000Z" }],
    };
    const local = {
      notes: [{ id: "n1", title: "Local edit", updatedAt: "2026-01-02T00:00:00.000Z" }],
    };
    const remote = {
  ...
  it("resolves a same-field edit conflict on a task using updatedAt, not just notes", () => {
    const base = {
      tasks: [
        { id: "t1", title: "Original", status: "todo", updatedAt: "2026-01-01T00:00:00.000Z" },
      ],
    };
    const local = {
      tasks: [
        { id: "t1", title: "Local edit", status: "todo", updatedAt: "2026-01-02T00:00:00.000Z" },
      ],
    };
    const remote = {
      tasks: [
        { id: "t1", title: "Remote edit", status: "todo", updatedAt: "2026-01-03T00:00:00.000Z" },
      ],
    };
  ...
  it("dwa urządzenia rozwijające tę samą serię nie tworzą duplikatów (deterministyczne id seriesId#index)", () => {
    ...
    const base = {
      tasks: [{ id: "s1#0", title: "Sprzątanie", status: "todo", seriesId: "s1", seriesIndex: 0 }],
    };
    ...
    const merged = mergeWorkspaceChanges(base, deviceA, deviceB) as {
