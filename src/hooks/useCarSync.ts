// Silnik synchronizacji modułu Auto — wzorowany na src/hooks/useFinanceSync.ts, ale
// operuje na src/store/useCarStore.ts (kolejka mutacji z idempotencją zamiast jednego
// dokumentu z rewizją). Patrz docs/plans/auto-car.md ("Frontend — dedykowany store + silnik sync").
//
// Przy montażu: GET /api/v1/car (hydratacja snapshotem). Jeśli w kolejce są niewysłane
// mutacje (offline edycje z poprzedniej sesji), najpierw je wysyła (POST /api/v1/car/mutations)
...
import { useCarStore, type CarMutationResult, type CarSnapshot } from "../store/useCarStore";

export type CarSyncState = "synced" | "saving" | "offline";
...
export function useCarSync(onSessionExpired?: () => void): { syncState: CarSyncState } {
  const [syncState, setSyncState] = useState<CarSyncState>("saving");
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        const snapshot = await apiRequest<CarSnapshot>("/api/v1/car", {
          signal: controller.signal,
        });
        if (!mounted.current || controller.signal.aborted) return;
        useCarStore.getState().hydrateFromSnapshot(snapshot);
        if (!mounted.current || controller.signal.aborted) return;
        setSyncState("synced");
      } catch (error) {
        if (!mounted.current || controller.signal.aborted) return;
      ...
      try {
        ...
        while (mounted.current && !controller.signal.aborted) {
          while (
            ...
            totalRounds += 1;
            setSyncState("saving");
            const batch = useCarStore.getState().pendingMutations;
            let response: { results: CarMutationResult[]; serverAt: string };
            try {
              response = await apiRequest<{ results: CarMutationResult[]; serverAt: string }>(
                "/api/v1/car/mutations",
                {
                  method: "POST",
                  json: {
            ...
            if (!mounted.current || controller.signal.aborted) return;
            useCarStore.getState().applyMutationResults(response.results);
          }
          if (!mounted.current || controller.signal.aborted) return;
          if (useCarStore.getState().pendingMutations.length > 0) {
    ...
    const resumeSync = () => {
      if (useCarStore.getState().pendingMutations.length > 0) void flush();
      else void hydrate();
    };
    ...
    // czytają zawsze świeży stan przez useCarStore.getState(), nie przez domknięcie.
  }, []);
