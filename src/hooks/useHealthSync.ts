// Silnik synchronizacji modułu Zdrowie — wzorowany na src/hooks/usePetsSync.ts, operuje na
// src/store/useHealthStore.ts (kolejka mutacji z idempotencją zamiast jednego dokumentu z
// rewizją). Patrz docs/plans/zdrowie-sql.md ("Frontend — dedykowany store + silnik sync").
//
// Przy montażu: GET /api/v1/health (hydratacja snapshotem). Jeśli w kolejce są niewysłane mutacje
// (offline edycje z poprzedniej sesji), najpierw je wysyła (POST /api/v1/health/mutations) i
...
export type HealthSyncState = "synced" | "saving" | "offline";
...
export function useHealthSync(onSessionExpired?: () => void): { syncState: HealthSyncState } {
  const [syncState, setSyncState] = useState<HealthSyncState>("saving");
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        const snapshot = await apiRequest<HealthSnapshot>("/api/v1/health", {
          signal: controller.signal,
        });
        if (!mounted.current || controller.signal.aborted) return;
        useHealthStore.getState().hydrateFromSnapshot(snapshot);
        if (!mounted.current || controller.signal.aborted) return;
        setSyncState("synced");
      } catch (error) {
        if (!mounted.current || controller.signal.aborted) return;
      ...
      try {
        ...
        while (mounted.current && !controller.signal.aborted) {
          while (
            ...
            useHealthStore.getState().pendingMutations.length > 0 &&
            totalRounds < MAX_FLUSH_ROUNDS
          ) {
            totalRounds += 1;
            setSyncState("saving");
            const batch = useHealthStore.getState().pendingMutations;
            let response: { results: HealthMutationResult[]; serverAt: string };
            try {
              response = await apiRequest<{ results: HealthMutationResult[]; serverAt: string }>(
                "/api/v1/health/mutations",
                {
                  method: "POST",
                  json: {
            ...
            if (!mounted.current || controller.signal.aborted) return;
            useHealthStore.getState().applyMutationResults(response.results);
          }
          if (!mounted.current || controller.signal.aborted) return;
          if (useHealthStore.getState().pendingMutations.length > 0) {
    ...
    const resumeSync = () => {
      if (useHealthStore.getState().pendingMutations.length > 0) void flush();
      else void hydrate();
    };
    ...
    // czytają zawsze świeży stan przez useHealthStore.getState(), nie przez domknięcie.
  }, []);
