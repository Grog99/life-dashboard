// Silnik synchronizacji modułu Zwierzęta — wzorowany na src/hooks/useCarSync.ts, operuje na
// src/store/usePetsStore.ts (kolejka mutacji z idempotencją zamiast jednego dokumentu z rewizją).
// Patrz docs/plans/zwierzeta-sql.md ("Frontend — dedykowany store + silnik sync").
//
// Przy montażu: GET /api/v1/pets (hydratacja snapshotem). Jeśli w kolejce są niewysłane mutacje
// (offline edycje z poprzedniej sesji), najpierw je wysyła (POST /api/v1/pets/mutations) i dopiero
...
import { usePetsStore, type PetsMutationResult, type PetsSnapshot } from "../store/usePetsStore";

export type PetsSyncState = "synced" | "saving" | "offline";
...
export function usePetsSync(onSessionExpired?: () => void): { syncState: PetsSyncState } {
  const [syncState, setSyncState] = useState<PetsSyncState>("saving");
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        const snapshot = await apiRequest<PetsSnapshot>("/api/v1/pets", {
          signal: controller.signal,
        });
        if (!mounted.current || controller.signal.aborted) return;
        usePetsStore.getState().hydrateFromSnapshot(snapshot);
        if (!mounted.current || controller.signal.aborted) return;
        setSyncState("synced");
      } catch (error) {
        if (!mounted.current || controller.signal.aborted) return;
      ...
      try {
        ...
        while (mounted.current && !controller.signal.aborted) {
          while (
            ...
            totalRounds += 1;
            setSyncState("saving");
            const batch = usePetsStore.getState().pendingMutations;
            let response: { results: PetsMutationResult[]; serverAt: string };
            try {
              response = await apiRequest<{ results: PetsMutationResult[]; serverAt: string }>(
            ...
            if (!mounted.current || controller.signal.aborted) return;
            usePetsStore.getState().applyMutationResults(response.results);
          }
          if (!mounted.current || controller.signal.aborted) return;
          if (usePetsStore.getState().pendingMutations.length > 0) {
    ...
    const unsubscribe = usePetsStore.subscribe((state) => {
      if (state.pendingMutations.length > 0) void flush();
    });

    const resumeSync = () => {
      if (usePetsStore.getState().pendingMutations.length > 0) void flush();
      else void hydrate();
    };
    ...
    // czytają zawsze świeży stan przez usePetsStore.getState(), nie przez domknięcie.
  }, []);
