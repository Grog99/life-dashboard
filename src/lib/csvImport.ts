...
function findHeader(headers: string[], patterns: RegExp[], fallback = ""): string {
  return (
    headers.find((header) => patterns.some((pattern) => pattern.test(header.toLocaleLowerCase("pl")))) ??
    fallback
  );
}
...
export function previewCsv(text: string): CsvPreview {
  ...
  if (lines.length - 1 > 10_000) throw new Error("Jednorazowo można zaimportować do 10 000 operacji");
  ...
  const amountColumn = findHeader(
    headers.filter((header) => !creditPatterns.some((pattern) => pattern.test(header.toLocaleLowerCase("pl")))),
    [/kwota/, /amount/, /wartość/, /obciążenie/],
    headers[1] ?? headers[0],
  );
  const creditColumn = findHeader(headers, creditPatterns);
  const titleColumn = findHeader(headers, [/tytuł/, /opis/, /description/, /szczegóły/], headers[2] ?? headers[0]);
...
export function mapCsvRows(
  ...
  return preview.rows
    .map((row) => {
      ...
      const creditMinor = mapping.creditColumn ? parseMoneyToMinor(row[mapping.creditColumn] ?? "") : 0;
      ...
      const category = (mapping.categoryColumn ? row[mapping.categoryColumn] : "")?.trim() || "Do przypisania";
