...
test("removing yourself is rejected regardless of role", () => {
  assert.throws(
    () => assertRemovableMember({ targetUserId: "user-1", targetRole: "member", sessionUserId: "user-1" }),
...
test("removing the owner is rejected", () => {
  assert.throws(
    () => assertRemovableMember({ targetUserId: "user-2", targetRole: "owner", sessionUserId: "user-1" }),
...
test("removing a member or admin who is not the caller is allowed", () => {
  assert.doesNotThrow(() =>
    assertRemovableMember({ targetUserId: "user-2", targetRole: "member", sessionUserId: "user-1" }),
  );
  assert.doesNotThrow(() =>
